...
def get_upsample_layer( #nontrainable:使用固定的、非学习的方法进行上采样;deconv:通过转置卷积层实现上采样;pixelshuffle:通过像素重排实现上采样
...
class ResBlock(nn.Module): #ResBlock不改变shape
    def __init__(
        self,
        spatial_dims: int,
        in_channels: int,
...
class uC_SegResNet(nn.Module):
    def __init__(
        ...
        self.convInit = get_conv_layer(spatial_dims, in_channels, init_filters)#初始卷积，将通道数由1变为8
        self.down_layers = self.make_down_layers()#下采样
    ...
    def make_down_layers(self):  #下采样
        ...
        for i, item in enumerate(blocks_down):
            ...
            pre_conv = (
                get_conv_layer(spatial_dims, layer_in_channels // 2, layer_in_channels, stride=2)#增加一倍channels，减少一倍spatial size;channels=(8,16,32->16,32,64)
                if i > 0
                else nn.Identity()
            )
    ...
    def make_up_layers(self):
        ...
        n_up = len(blocks_up) #上采样3次(1,1,1)
        for i in range(n_up):
            ...
            up_samples.append(
                nn.Sequential(
                    *[
                        ...
                        get_upsample_layer(spatial_dims, sample_in_channels // 2, upsample_mode=upsample_mode), #特征图的大小*2
                    ]
                )
            )
            up_layers.append(
                nn.Sequential(
                    *[
                        ResBlock(spatial_dims, sample_in_channels // 2, norm=norm, act=self.act) #ResBlock3次
    ...
    def make_final_conv(self, out_channels: int): #最后一个GroupNorm,ReLU,Conv
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        x = self.convInit(x)#初始卷积，将通道数由1变为32
        # print(x.shape)
        if self.dropout_prob is not None:   #Dropout层
            x = self.dropout(x)

        down_x = []

        for i,down in enumerate(self.down_layers): #总共4个阶段,分别通过1，2，2，4次ResBlock，第1个阶段无前置下采样，其余各阶段均有前置下采样操作
            x = down(x)
            ...
            # if self.dropout_prob is not None:   #Dropout层
            #     x = self.dropout(x)
            down_x.append(uc_skip) #down_x即为跳跃连接的对象
        # print(down_x[0].shape,down_x[1].shape,down_x[2].shape,down_x[3].shape)
        return x, down_x

    def decode(self, x: torch.Tensor, down_x: list[torch.Tensor]) -> torch.Tensor:
        for i, (up, upl) in enumerate(zip(self.up_samples, self.up_layers)): #up对应up_samples层(上采样)，upl对应up_layers层(ResBlock)
            x = up(x) + down_x[i + 1] #上采样，跳跃连接
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:#设输入为(4,1,96,96,96);96->48->24->12->6
